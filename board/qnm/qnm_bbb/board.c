...
#include "../../ti/common/cape_detect.h"
#include "board.h"
...
static int qnm_boot_mmc_dev(void)
{
...
static const char *qnm_boot_source_name(void)
{
	switch (gd->arch.omap_boot_device) {
	case BOOT_DEVICE_MMC1:
		return "sd";
...
{
	return qnm_boot_mmc_dev();
}

int checkboard(void)
{
	if (board_is_qnm_bbb()) {
		const char *rev = board_ti_get_rev();

		if (rev && *rev && strcmp(rev, "unknown"))
			printf("Board: QNM_BBB rev %s\n", rev);
		else
			puts("Board: QNM_BBB\n");
	}
	else if (board_ti_get_name())
		printf("Board: %s\n", board_ti_get_name());
	else
		puts("Board: unknown\n");
...
#if !defined(CONFIG_XPL_BUILD)
static const char *qnm_boot_rom_name(void)
{
	switch (gd->arch.omap_boot_device) {
	case BOOT_DEVICE_MMC1:
		return "MMC1";
...
static int do_bootsrc(struct cmd_tbl *cmdtp, int flag, int argc,
		      char *const argv[])
{
	printf("boot_source : %s\n", qnm_boot_source_name());
	printf("rom_boot    : %s\n", qnm_boot_rom_name());
	printf("u-boot_mmc  : mmc %d\n", qnm_boot_mmc_dev());
	printf("env_mmc     : mmc %d\n", mmc_get_env_dev());
	puts("mapping     : ROM MMC1 -> U-Boot mmc 0 (SD), ROM MMC2 -> U-Boot mmc 1 (eMMC)\n");

	return CMD_RET_SUCCESS;
}

U_BOOT_CMD(
	bootsrc, 1, 1, do_bootsrc,
	"show boot source mapping",
	"- display whether the board booted from SD or eMMC and show ROM/U-Boot numbering"
);
#endif
...
const struct dpll_params *get_dpll_ddr_params(void)
{
	...
	else if (board_is_pb() || board_is_bone_lt_family() || board_is_icev2())
...
const struct dpll_params *get_dpll_mpu_params(void)
{
	...
	if (board_is_pb() || board_is_bone_lt_family())
		freq = MPUPLL_M_1000;
...
void sdram_init(void)
{
	...
	else if (board_is_pb() || board_is_bone_lt_family())
...
int board_late_init(void)
{
	...
	if (board_is_qnm_bbb()) {
		name = "QNM_BBB";
		env_set("fdtfile", "am335x-qnm-bbb.dtb");
	...
	env_set("boot_source", qnm_boot_source_name());
	env_set_ulong("boot_mmcdev", qnm_boot_mmc_dev());
	printf("Boot source: %s (mmc %d)\n",
	       qnm_boot_source_name(), qnm_boot_mmc_dev());
...
int board_fit_config_name_match(const char *name)
{
	if (board_is_gp_evm() && !strcmp(name, "am335x-evm"))
		...
		return 0;
	else if (board_is_qnm_bbb() && !strcmp(name, "am335x-qnm-bbb"))
		return 0;
